...
namespace Monetization_Automation.Test
{
    ...
    public class D_SaveNetworkrStatsAPIInputGenerator
    {
        ...
        public void D_SaveNetworkrStatsMainMethodCodeGenerator()
        {
            ...
            DataCollectionAPIKeys.URLSDK = Utils.ExcelUtil.ReadData(3, "URL");
            Extension.OpenDBConnection();
            QueryArrayAllAPI.queryArrayAllAPI[0] = Utils.ExcelUtil.ReadData(3, "Query") + " " + Utils.ExcelUtil.ReadData(1, "Limit");
            QueryArrayAllAPI.queryArrayAllAPI[1] = Utils.ExcelUtil.ReadData(3, "adID") + " " + Utils.ExcelUtil.ReadData(1, "Limit");
            Extension.ExecuteQueryAPICodeGenerator(SqlConnectionInstance.SqlConnection, dataSetIndex);
            CreateJsonRequestInputGenerator.CreateJson(DataSetPropertyFill.dataSetPropertyFill, "SaveNetworkStats");
            Thread.Sleep(2000);
        }
    }
}
