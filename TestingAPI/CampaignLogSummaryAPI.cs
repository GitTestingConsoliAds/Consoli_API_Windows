...
{
    [TestClass]
    public class CampaignLogSummaryAPI
    {
        
        public CampaignLogSummaryAPI()
        {
            try
            {
                if (ExcelUtil._dataCollection.Count == 0)
                {
                    ...
                    if (directoryInfo != null)
                    {
                        ...
                        {
                            ExcelUtil.PopulateInCollection(@"/Volumes/DataDisk/Ahmadkhisal/Consoliads/Automation_Data/APIExcels/CampaignLogSummary.xlsx", Monetization_Automation.TestingAPI.Properties.Settings.Default.Environment);
                        }
                    }
                }
            }
        ...
        [TestMethod]
        public void CampaignLogSummaryMainMethod()
        {
            ...
            foreach (System.Data.DataRow row in DataTableCollectionLoop.tableCollection.Rows)
            {
                ...
                request.AddParameter("text/plain", jSonbody, ParameterType.RequestBody);
                SendJsonRequest.SendJsonRequestMainMethod(jSonbody, DataCollectionAPIKeys.URLSDK, request,count, "CampaignLogSummary");
                count = count - 1;
            }
        }

        //[TestMethod]
        public void CampaingLogSummaryReuseTokenSendtoDBMainMethod()
        {
            int count = DataTableCollectionLoop.tableCollection.Rows.Count;
            foreach (System.Data.DataRow row in DataTableCollectionLoop.tableCollection.Rows)
            {
                string jSonbody = Utils.ExcelUtil.ReadData(count, "JSON");
                string URL = Utils.ExcelUtil.ReadData(count, "URL");
