...
{
    [TestClass]
    public class E_StartNewAdSessionInputGenerator
    {
        
        public E_StartNewAdSessionInputGenerator()
        {
        ...
        public void E_StartNewAdSessionMainInputGenerator()
        {
            int test = 0;
            int dataSetIndex = 1;
            Extension.OpenDBConnection();
            QueryArrayAllAPI.queryArrayAllAPI[0] = Utils.ExcelUtil.ReadData(6, "Query") + " " + Utils.ExcelUtil.ReadData(1, "Limit");
            Extension.ExecuteQueryAPICodeGenerator(SqlConnectionInstance.SqlConnection, dataSetIndex);
            CreateJsonRequestInputGenerator.CreateJson(DataSetPropertyFill.dataSetPropertyFill, "StartNewAdSession");
            Thread.Sleep(2000);
        }
    }
}