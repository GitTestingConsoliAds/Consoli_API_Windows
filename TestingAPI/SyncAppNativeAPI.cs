...
{
    [TestClass]
    public class SyncAppNativeAPI
    {
        
        public SyncAppNativeAPI()
        {
            try
            {
                if (ExcelUtil._dataCollection.Count == 0)
                {
                    ...
                    if (directoryInfo != null)
                    {
                        ...
                        {
                            ExcelUtil.PopulateInCollection(@"/Volumes/DataDisk/Ahmadkhisal/Consoliads/Automation_Data/APIExcels/SyncAppNative.xlsx", Monetization_Automation.TestingAPI.Properties.Settings.Default.Environment);
                        }
                    }
                }
            }
     ...
     //   [TestMethod]
        public void SyncAppNativeAPIMainMethod()
        {
            ...
            foreach (System.Data.DataRow row in DataTableCollectionLoop.tableCollection.Rows)
            {
                ...
                SendJsonRequest.SendJsonRequestMainMethod(jSonbody, DataCollectionAPIKeys.URLSDK, request, count, "SyncAppNative");
                count = count - 1;
            }
        }
    }
